...
1.Read both training and testing instances.

2.The function LOAD is to load the packages.

```{r}
setwd("C:/aaaaaaaaaaaaa/coursera/Data science/8.Practical machine learning")
...
1.To drop columns with NAs, drop highly correlated variables and drop variables with 0 (or approx to 0) variance.

2.The results are hidden as they take a very long space.
...
To split the sample in two samples. 60% for training and 40% for testing is the usual.
...
1.A predictive model is fitted using Random Forest algorithm. Highly correlated variables were already removed but still this algorithm is robust to correlated covariates and outliers.

2.A 5 fold cross validation is used.
